function [predicted,memberships,numhits,indeterm_bon,indeterm_mauv] = fknn(data, labels, test, ...
...
% allocate space for 'indeterm_bon' et 'indeterm_mauv'
indeterm_bon = zeros(length(k_values),1);
indeterm_mauv = zeros(length(k_values),1);
...
% 			num2str(i) '. These weights are set to 1.']);
    ...
    % comptabilise les signaux qui ont une probabilité quasi égale
    % d'appartenir à plusieurs classes
    test_out2=test_out/max(test_out);
    if length(find(test_out2(:)>=0.8))>1
        if ~isempty(testlabels) && predicted(i,k)==testlabels(i)
        indeterm_bon(k)=indeterm_bon(k)+1; %Indétermination mais bien classé
        else
        indeterm_mauv(k)=indeterm_mauv(k)+1; %Indétermination et mal classé
        end
    end
